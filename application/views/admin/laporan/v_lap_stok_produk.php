...
<?php
    ...
    if($group=='-' || $group!=$d['kategori_nama']){
        $kat=$d['kategori_nama'];
        $query=$this->db->query("SELECT kategori_id,kategori_nama,produk_nama,SUM(produk_stok) AS tot_stok FROM tbl_kategori JOIN tbl_produk ON kategori_id=produk_kategori_id WHERE kategori_nama='$kat'");
        $t=$query->row_array();
        $tots=$t['tot_stok'];
        if($group!='-')
        echo "</table><br>";
...
$nomor=1;
    ...
    $group=$d['kategori_nama'];
        ...
        <tr>
                ...
                <td style="vertical-align:top;text-align:center;"><?php echo $d['produk_stok']; ?></td>
        </tr>
